...
import { detectTabPages } from '@/app/lib/pdf-utils';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Detect tab pages for user review
    const detectionResult = await detectTabPages(buffer);
    ...
    let errorMessage = 'Failed to detect tab pages';
    if (error instanceof Error) {
      if (error.message.includes('bad XRef entry') || error.message.includes('XRef')) {
        errorMessage = 'PDF file appears to be corrupted or damaged. Tab detection will use heuristics.';
